...
import (
	...
	sortedqueue "github.com/beka-birhanu/vinom/api/infrastruture/sortedqueue"
	"github.com/beka-birhanu/vinom/api/infrastruture/token"
	auth_service "github.com/beka-birhanu/vinom/api/service/auth"
	gamesession_service "github.com/beka-birhanu/vinom/api/service/game"
	matchmaking_service "github.com/beka-birhanu/vinom/api/service/matchmaking"
...
var (
	...
	gameSessionManager    gamesession_service.SessionService
	userRepo              auth_service.UserRepo
	matchmakerInner       matchmaking_service.Service
	matchmakingController api_i.Controller
	jwtTokenizer          auth_service.Tokenizer
...
func initUDPSocketManager() {
	serverAddr, err := net.ResolveUDPAddr("udp", fmt.Sprintf(":%v", config.Envs.UdpPort))
	if err != nil {
	...
	server, err := udpsocket.NewServerSocketManager(
		...
		udpsocket.ServerWithHeartbeatExpiration(time.Duration(config.Envs.UDPHeartbeatExpiration)*time.Millisecond),
	)
	if err != nil {
...
func initGameSessionManager() {
	...
	if err != nil {
		appLogger.Error(fmt.Sprintf("Creating game session manager logger: %v", err))
		os.Exit(1)
	}

	manager, err := gamesession_service.NewSessionManager(
		&gamesession_service.Config{
...
func initMatchmaker() {
	sq, err := sortedqueue.NewRedisUniqueSortedQueue(redisClient, 300)
	if err != nil {
	...
	opts := &matchmaking_service.Options{
	...
	matchmaker, err := matchmaking_service.NewService(sq, matchLogger, opts)
	if err != nil {
		appLogger.Error(fmt.Sprintf("Creating matchmaker: %v", err))
		os.Exit(1)
	}

	matchmakerInner = matchmaker
	appLogger.Info("Matchmaker initialized")
}

func initMatchmakingController() {
	var err error
	matchmakingController, err = gameapi.NewMatchMakingController(gameSessionManager, userRepo, matchmakerInner)
	if err != nil {
...
func initAuthService() {
	var err error
	authService, err = auth_service.NewService(userRepo, jwtTokenizer)
	if err != nil {
...
func initRouter(t auth_service.Tokenizer) {
	router = api.NewRouter(api.Config{
		Addr:                    fmt.Sprintf(":%v", config.Envs.RESTPort),
		BaseURL:                 "/api",
