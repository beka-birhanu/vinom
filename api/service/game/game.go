...
// gameService represents a maze game with players, a maze, and game state.
...
// NewGameService creates a new Game instance with the specified maze, players, and encoder.
// Returns an error if configuration constraints are violated.
func NewGameService(maze game_i.Maze, players []game_i.Player, e game_i.GameEncoder) (*gameService, error) {
...
func (g *gameService) Start(ctx context.Context, gameDuration time.Duration) {
...
func (g *gameService) handleAction(t byte, move []byte) {
	switch t {
...
func (g *gameService) broadcastState(ended bool) {
	defer g.Wg.Done()
...
func (g *gameService) snapshot() game_i.GameState {
	g.RLock()
	defer g.RUnlock()
...
func (g *gameService) handleIncomingMove(a game_i.Action) {
	g.Lock()
...
func (g *gameService) StateChan() <-chan []byte {
	return g.stateChan
}

// ActionChan returns the action channel.
func (g *gameService) ActionChan() chan<- []byte {
	return g.actionChan
}

// EndChan returns the end channel for the game.
func (g *gameService) EndChan() <-chan []byte {
	return g.endChan
}
