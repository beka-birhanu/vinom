...
var defaultPlayerPositions = []struct {
	row int32
	col int32
...
func NewSessionManager(c *Config) (SessionService, error) {
	gsm := &sessionService{
		socket:       c.Socket,
...
func (g *sessionService) NewSession(ctx context.Context, playerIDs []uuid.UUID) {
	...
	gameServer, err := NewGameService(maze, players, g.gameEndcoder)
	if err != nil {
	...
	go gameServer.Start(ctx, defaultGameDuration)
	go g.listenGameChan(sessionID)
	g.logger.Info(fmt.Sprintf("started new game for players: %v", playerIDs))
}

func (g *sessionService) SessionInfo(ctx context.Context, playerID uuid.UUID) ([]byte, string, error) {
	g.RLock()
	defer g.RUnlock()
...
func (g *sessionService) Authenticate(s []byte) (uuid.UUID, error) {
	g.RLock()
	defer g.RUnlock()
...
func (g *sessionService) saveSession(players []uuid.UUID, gs GameService) uuid.UUID {
	sessionID := uuid.New()
	for {
...
func (g *sessionService) listenGameChan(id uuid.UUID) {
...
func (g *sessionService) writePlayerRequest(pID uuid.UUID, actionType byte, payload []byte) {
	g.RLock()
	defer g.RUnlock()
...
func (g *sessionService) clean(ID uuid.UUID) {
	g.Lock()
	defer g.Unlock()
